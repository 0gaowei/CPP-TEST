...
## 附录：无需 Parasoft 的自动化方案

如果当前机器暂未安装 Parasoft，可以直接运行仓库自带的脚本完成“自动生成测试 → 覆盖率统计 → 静态分析 → 报告导出”：

```bash
python3 tools/simple_quality_suite.py
```

- 自动生成的测试文件保存为 `tests/generated_tests.cpp`（配套元数据 `tests/generated_tests.json`）。
- 运行结果写入 `reports/tests/results.json`，包含总用例数、失败数和失败详情。
- 覆盖率数据存放在 `reports/coverage/summary.json`（语句/分支/函数覆盖率）以及 `reports/coverage/calculator.cpp.gcov`。
- 静态分析结果位于 `reports/static/static_analysis.json`。脚本优先调用 `cppcheck`，若系统没有会退回到启发式检查并在 JSON 中标注 `tool: heuristic`。
- 综合 HTML/XML 报告位于 `reports/quality_summary.html`、`reports/quality_summary.xml`（默认被 `.gitignore` 忽略，可酌情复制到其他目录）。

脚本依赖 `g++/gcov`（系统默认即包含）。如需更换编译器可指定 `--compiler=clang++` 等参数；也可以用 `--skip-*` 系列参数选择性跳过某些阶段。
